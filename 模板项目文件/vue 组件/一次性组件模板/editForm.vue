<el-form-item label="规则模板"
  :rules="[{required: true}]"
>
  <el-select v-model="SQLmodelType" @change="handleSQLModelChange" placeholder="请选择" clearable>
...
<el-form-item v-bind:label="$version.jobManagement.weekdays">
<el-checkbox-group v-model="selectedWeekDays" >
<el-checkbox label = "Mon" name="selectedWeekDays" :disabled="radio === 'schduleByCron'">{{$version.jobManagement.mon}}</el-checkbox>
  <el-checkbox label = "Tue" name="selectedWeekDays" :disabled="radio === 'schduleByCron'">{{$version.jobManagement.tue}}</el-checkbox>
  <el-checkbox label = "Wed" name="selectedWeekDays" :disabled="radio === 'schduleByCron'">{{$version.jobManagement.wed}}</el-checkbox>
  <el-checkbox label = "Thu" name="selectedWeekDays" :disabled="radio === 'schduleByCron'">{{$version.jobManagement.thu}}</el-checkbox>
  <el-checkbox label = "Fri" name="selectedWeekDays" :disabled="radio === 'schduleByCron'">{{$version.jobManagement.fri}}</el-checkbox>
  <el-checkbox label = "Sat" name="selectedWeekDays" :disabled="radio === 'schduleByCron'">{{$version.jobManagement.sat}}</el-checkbox>
  <el-checkbox label = "Sun" name="selectedWeekDays" :disabled="radio === 'schduleByCron'">{{$version.jobManagement.sun}}</el-checkbox>
</el-checkbox-group>
</el-form-item>
<el-form-item v-bind:label="$version.jobManagement.time">
<el-row  :gutter="10">
  <el-col :span="3" >
    <el-select name="startTimeHour" v-model="selectedHour" :placeholder="$version.jobManagement.hourDefault" :disabled="radio === 'schduleByCron'">
        <!-- <el-option v-bind:value="hour" >0</el-option> -->
      <el-option v-for="hour in 24" :key='hour.id' v-bind:value="hour-1" >{{hour-1}}</el-option>
    </el-select>
  </el-col>
  <el-col :span="1">:</el-col>
  <el-col :span="3">
    <el-select name="startTimeMinute" v-model="selectedMin" :placeholder="$version.jobManagement.minuteDefault" :disabled="radio === 'schduleByCron'">
      <el-option v-for="minute in 60" :key='minute.id' v-bind:value="minute-1">{{minute-1}}</el-option>
    </el-select>
  </el-col>
</el-row>
</el-form-item>
...
this.$http.get(this.$url + '/service/models/fromre/')
.then(res => {
let data = res.data;
...
this.$http.post(this.$url + '/service/entities/search', json)
.then(res => {
  if(res.data && (res.data.totalItems || res.data.totalItems === 0)) {
...
this.tableName = this.tableMap[tableId] ? this.tableMap[tableId].physicalName : '';
if (this.modelFromDam) {
  let callback = (data) => {
...
this.columnName2 = '';
if (tableId || tableId === 0) {
this.tableName2 = this.tableMap[tableId] ? this.tableMap[tableId].physicalName : '';
if (this.modelFromDam) {
  let callback = (data) => {
...
loadColumn(callback) {
this.$http.get(this.$url + '/service/entities/' + this.tableId + '/columns')
.then(res => {
...
this.$http.get(this.$url + '/service/models/ddm/models/' + this.modelId + '/table/' + tableId)
.then(res => {
...
this.$http.get(this.$url + '/service/entities/' + tableId + '/columns')
.then(res => {
if (res.data && Array.isArray(res.data)) {
  callback && callback(res.data);
}
...
this.columnName = this.columnMap[columnId] ? this.columnMap[columnId].physicalName : '';
},
handleColumnChange2(columnId) {
this.columnId2 = columnId;
this.columnName2 = this.columnMap2[columnId] ? this.columnMap2[columnId].physicalName : '';
},
...
// this.$http.get()
let url = `${this.$url}/service/domains/codes/content?codeNumber=${newVal[1]}`;
newVal[1] && this.$http.get(url)
.then(res => {
if (res.data) {
...
if (this.modelId || this.modelId === 0) {
  let callback = (data) => {
    ...
    this.tableName = this.tableMap[this.tableId] ? this.tableMap[this.tableId].physicalName : '';
    if (this.tableId || this.tableId === 0) {
      let callback = (data) => {
        ...
        this.columnName = this.columnMap[this.columnId] ? this.columnMap[this.columnId].physicalName : '';
      };
      this.loadColumn(callback);
    }
  };
...
if (this.SQLmodelType=== '1') {
  ...
  switch (this.rangeType) {
    ...
    break;
    case '区间':
    let arr = [this.rangeValue, this.rangeValueMax];
    if (isNum(arr[0]) && isNum(arr[1]) && arr[0] <= arr[1]) {
      result = result + ' ( ' + tableName + '.' + columnName + ' >= ' + (arr[0] - 0) + ' AND ' + tableName + '.' + columnName + ' <= ' +  (arr[1] - 0) + ' )';
    } else {
      errMes = '请输入正确的范围';
    }
    break;
  ...
  if (this.rangeType !== '区间') {
    let bool = isNum(this.rangeValue);
    if (!bool) {
      errMes = '请在值域内输入数字';
    }
    result = result + tableName + '.' + columnName + ' ' + operate + ' ' + (this.rangeValue-0);
  }
  ...
  // 唯一性
  result += ' WHERE ' + columnName + ' IN ( SELECT ' + columnName + ' FROM ' + tableName + ' GROUP BY ' + columnName + ' HAVING COUNT( ' + columnName + ' ) > 1 ) ORDER BY ' + columnName;
  ...
  if (!columnName)
  {
    if(this.columnsArr && this.columnsArr.length > 0)
    {
      let str = '';
      this.columnsArr.forEach(item => {
        str += '(' + item.physicalName + ' IS NULL) OR ( ' + item.physicalName + " = '') OR ";
      });
      str = str.slice(0, -4);
      result += ' WHERE (' + str + ')';
    }
  ...
  {
    result = result + ' WHERE ( (' + tableName + '.' + columnName + ' IS NULL ) OR ( ' + tableName + '.' + columnName + " =  '' )" + ')';
  }
} else if (this.SQLmodelType=== '5') {
  let tableName2 = this.tableName2 ? this.tableName2 : '';
  let columnName2 = this.columnName2 ? this.columnName2 : '';
  // 主外键检查
  if (!tableName2) {
    errMes = '请选择外键关联表';
  }
  ...
  result += ' WHERE ' + tableName + '.' + columnName + ' NOT IN ( SELECT ' + columnName2 + ' FROM ' + tableName2 + ' )';
} else if (this.SQLmodelType=== '6') {
  // 字段长度检查
  if(!this.DBtype) {
    errMes = '请选择数据库类型';
  }
  let funName = this.dbTypeLengthMap[this.DBtype] || 'LENGTH';
  result += ' WHERE ' + funName + '( ' + columnName + ' ) > ' + this.lengthValue;
  ...
  str = str.slice(0, -2)
  result += ' WHERE ' + tableName + '.' + columnName + ' NOT IN ' + '(' + str + ')';
}
} else {
if (this.dateType === 1) {
  ...
  result = `[0-9]{4}\-[01][0-9]\-[0-3][0-9] [0-2][0-9]\:[0-5][0-9]\:[0-5]{1}[0-9]{1}`;
}
}
...
getCodeData() {
this.$http.get(this.$url + '/service/domains/codes')
.then(res => {
if (res.data && Array.isArray(res.data)) {
...
this.$http.get(this.$url + '/service/models/ddm/models/' + modleId + '/tables')
.then(res => {
if(res.data && Array.isArray(res.data)) {
  callabck && callabck(res.data);
}
